...
fn test_1aln_roundtrip_preserves_data() -> Result<()> {
    ...
    if aln_result.is_err() {
        eprintln!("Skipping test - FastGA alignment failed (expected when running from temp directories)");
        return Ok(());
    }
    ...
    assert_eq!(filtered_count, original_count, 
    ...
    for (i, (orig, filt)) in original_meta.iter().zip(filtered_meta.iter()).enumerate() {
        assert_eq!(orig.query_name, filt.query_name, 
            "Record {} query name mismatch", i);
        assert_eq!(orig.target_name, filt.target_name,
            "Record {} target name mismatch", i);
        assert_eq!(orig.query_start, filt.query_start,
            "Record {} query_start mismatch", i);
        assert_eq!(orig.query_end, filt.query_end,
            "Record {} query_end mismatch", i);
        assert_eq!(orig.target_start, filt.target_start,
            "Record {} target_start mismatch", i);
        assert_eq!(orig.target_end, filt.target_end,
            "Record {} target_end mismatch", i);
        assert_eq!(orig.strand, filt.strand,
            "Record {} strand mismatch", i);
        assert_eq!(orig.matches, filt.matches,
            "Record {} matches mismatch", i);
        
        // Allow small floating point differences in identity
        let identity_diff = (orig.identity - filt.identity).abs();
        assert!(identity_diff < 0.001,
            "Record {} identity diff too large: {} vs {} (diff: {})",
            i, orig.identity, filt.identity, identity_diff);
    }
...
fn test_1aln_roundtrip_with_filtering() -> Result<()> {
    ...
    assert!(filtered_meta.len() < original_meta.len(),
    ...
    for filt in &filtered_meta {
        ...
        assert!(matching_orig.is_some(),
            "Filtered record not found in original: {} → {} [{}-{}, {}-{}]",
            filt.query_name, filt.target_name,
            filt.query_start, filt.query_end,
            filt.target_start, filt.target_end);
    }
