...
use sweepga::plane_sweep_exact::{PlaneSweepMapping, plane_sweep_query, plane_sweep_target, plane_sweep_both};

/// Helper function to create a mapping
fn make_mapping(idx: usize, q_start: u32, q_end: u32, t_start: u32, t_end: u32) -> PlaneSweepMapping {
    PlaneSweepMapping {
        idx,
        query_start: q_start,
        query_end: q_end,
...
fn test_overlapping_mappings_keep_best() {
    ...
    let kept = plane_sweep_query(&mut mappings, 0, 0.95);
    assert_eq!(kept.len(), 2, "Both overlapping mappings are best at different positions");
}
...
fn test_identical_mappings() {
    ...
    // But when all have identical scores, tie-breaking keeps all at same score level
    let kept = plane_sweep_query(&mut mappings, 0, 0.95);
    // All three mappings have identical query ranges and identical scores
    // The plane sweep keeps all mappings that are "best" at any position
    // Since they all have the same score, they're all equally "best"
    assert_eq!(kept.len(), 3, "All mappings with identical scores are kept");

    // With n=1 (best + 1 secondary), all should still be kept
    // because they all have the same score
    mappings.iter_mut().for_each(|m| m.flags = 0);  // Reset flags
    let kept = plane_sweep_query(&mut mappings, 1, 0.95);
    assert_eq!(kept.len(), 3, "All mappings with identical scores kept even with n=1");
...
fn test_overlap_threshold() {
    ...
    // But all 4 mappings have identical query ranges and same scores
    // The overlap filtering happens AFTER selecting best+secondaries
    // Since all 4 have the same score, they're all treated as "best"
    let kept = plane_sweep_query(&mut mappings, 1, 0.5);
    assert_eq!(kept.len(), 4, "All mappings with identical scores kept");
...
fn test_complex_overlaps() {
    ...
    assert!(kept.len() >= 3, "Multiple mappings kept in complex overlap pattern");
}
...
fn test_both_axes_filtering() {
    ...
    assert!(kept.contains(&3), "Non-overlapping mapping kept in both axes");
}
...
fn test_score_calculation() {
    ...
    assert!(long_mapping.score() > short_mapping.score(),
...
fn test_real_world_scenario() {
    ...
    assert!(kept_with_secondary.len() > kept.len(),
