...
";
    ...
    assert_eq!(
        lines.len(),
        2,
        "Should keep best mapping per query sequence"
    );

    // Check that the longest mappings are kept
    assert!(
        result.contains("800\t800"),
        "Longest query1 mapping should be kept"
    );
    assert!(
        result.contains("1300\t1300"),
        "Longest query2 mapping should be kept"
    );
...
";
    ...
    assert_eq!(
        lines.len(),
        4,
        "All non-overlapping mappings should be kept with n=-1"
    );
...
";
    ...
    assert!(
        has_non_overlapping,
        "Non-overlapping mapping should be kept"
    );
...
";
    ...
    // Run with scaffolding disabled (just plane sweep, no scaffold creation)
    let output = Command::new("cargo")
        .args([
            "run",
            "--release",
        ...
        .output()
        .expect("Failed to run sweepga without scaffolding");

    assert!(output.status.success(), "No-scaffold run failed");

    let filtered = fs::read_to_string(output_filtered).expect("Failed to read filtered output");
    let no_scaffold =
        fs::read_to_string(output_unfiltered).expect("Failed to read no-scaffold output");

    let filtered_lines = filtered.lines().count();
    let no_scaffold_lines = no_scaffold.lines().count();

    // Both runs disable scaffolding and use plane sweep
    // With these mappings (non-overlapping), both should keep all 4
    assert_eq!(
        filtered_lines, 4,
        "Plane sweep should keep all non-overlapping mappings"
    );
    assert_eq!(no_scaffold_lines, 4, "No-scaffold should have all mappings");
