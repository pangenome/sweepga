...
fn test_identity_scoring_prefers_high_identity() {
    ...
    assert_eq!(kept[0], 1, "Should keep mapping with 99% identity, not the longest");
}
...
fn test_length_scoring_prefers_long_alignments() {
    ...
    assert_eq!(kept.len(), 1, "Should keep exactly 1 mapping");
    assert_eq!(kept[0], 1, "Should keep longest mapping (500bp), not highest identity");
}
...
fn test_length_identity_scoring_balances_both() {
    ...
    assert_eq!(kept.len(), 1, "Should keep exactly 1 mapping");
    assert_eq!(kept[0], 1, "Should keep mapping with highest length*identity product");
}
...
fn test_log_length_identity_scoring_dampens_length_effect() {
    ...
    assert_eq!(kept.len(), 1, "Should keep exactly 1 mapping");
    assert_eq!(kept[0], 2, "Should keep mapping with highest log(length)*identity score");
}
...
fn test_ranking_with_identical_scores() {
    ...
    let kept = plane_sweep_query(&mut mappings, 1, 0.95, ScoringFunction::LengthIdentity);
    assert_eq!(kept.len(), 1, "Should keep exactly 1 mapping even with ties");
...
fn test_overlapping_mappings_best_survives() {
    ...
    assert!(kept.contains(&2), "Best identity mapping (95%) should be kept");
}
...
fn test_scoring_with_contained_mappings() {
    ...
    assert!(kept.contains(&1), "Identity scoring should keep high-identity mapping");
    ...
    assert!(kept.contains(&0), "Length scoring should keep longer mapping");

    // Log-length-identity might prefer either depending on exact values
    // log(400) * 0.80 ≈ 4.79 vs log(100) * 0.99 ≈ 4.55
    mappings.iter_mut().for_each(|m| m.flags = 0);
    let kept = plane_sweep_query(&mut mappings, 1, 0.95, ScoringFunction::LogLengthIdentity);
    assert!(kept.contains(&0), "Log-length-identity should keep mapping with better combined score");
}
...
fn test_ranking_order_with_multiple_mappings() {
    ...
    assert!(kept.contains(&2), "Should keep highest scoring mapping (180)");
    assert!(kept.contains(&1), "Should keep second highest scoring mapping (120)");
}
...
fn test_extreme_values() {
    ...
    let kept = plane_sweep_query(&mut mappings.clone(), 1, 0.95, ScoringFunction::Length);
    assert_eq!(kept[0], 1, "Length scoring should prefer 100kb mapping despite 1% identity");

    // Identity scoring should strongly prefer the perfect match
    mappings.iter_mut().for_each(|m| m.flags = 0);
    let kept = plane_sweep_query(&mut mappings.clone(), 1, 0.95, ScoringFunction::Identity);
    assert_eq!(kept[0], 0, "Identity scoring should prefer 100% identity despite 1bp length");

    // Log-length-identity should be more balanced
    // log(1) * 1.0 = 0, log(100000) * 0.01 ≈ 0.115, log(1000) * 0.5 ≈ 3.45
    mappings.iter_mut().for_each(|m| m.flags = 0);
    let kept = plane_sweep_query(&mut mappings, 1, 0.95, ScoringFunction::LogLengthIdentity);
    assert_eq!(kept[0], 2, "Log-length-identity should prefer balanced mapping");
}